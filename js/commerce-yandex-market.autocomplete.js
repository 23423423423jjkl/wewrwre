...
// Registers the commerceYandexMarket namespace.
Drupal.commerceYandexMarket = Drupal.commerceYandexMarket || {};

(function($) {
  Drupal.behaviors.commerce_yandex_market_autocomplete = {
    attach: function(context) {
      var autocompleteSettings = Drupal.settings.commerce_yandex_market_autocomplete;

      $('input.commerce-yandex-market-autocomplete').once(function() {
        new Drupal.commerceYandexMarket.autocomplete(this, autocompleteSettings[$(this).attr('id')]);
      });
    }
  };

  /**
   * commerceYandexMarket autocomplete object.
   */
  Drupal.commerceYandexMarket.autocomplete = function(input, settings) {
    ...
    this.button.addClass('ui-corner-right ui-button-icon commerce-yandex-market-autocomplete-button');
    ...
    this.jqObject.autocomplete("widget").addClass('commerce-yandex-market-autocomplete');

    // Save the current commerceYandexMarket object, so it can be used in
    // handlers.
    var instance = this;
    ...
    this.jqObject.autocomplete("option", "source", function(request, response) {
      ...
      $.ajax( {
        url: instance.uri + '/' + encodeURIComponent(request.term),
        dataType: "json",
  ...
  /**
   * Success function for commerceYandexMarket autocomplete object.
   */
  Drupal.commerceYandexMarket.autocomplete.prototype.success = function(data, request, response) {
    var list = new Array();
   ...
   * @param searchFor
   *   The term for will be searched for. If undefined then the
   *   entered input text will be used.
   */
  Drupal.commerceYandexMarket.autocomplete.prototype.open = function(searchFor) {
   ...
   */
  Drupal.commerceYandexMarket.autocomplete.prototype.close = function() {
   ...
   */
  Drupal.commerceYandexMarket.autocomplete.prototype.toggle = function() {
